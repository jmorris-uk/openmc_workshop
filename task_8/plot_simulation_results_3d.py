...
with open('simulation_results4000.json') as f:
    results = json.load(f)
...
for tally_name in ['leak_tally','tbr_tally']:
    ...
    for material_name in all_materials:
        ...
        traces.append(Scatter3d(x=list(df_filtered_by_mat[x_axis_name]), 
                                ...
                                visible=False,
                                marker={'color':list(df_filtered_by_mat[tally_name]),
                                        'colorscale':'Viridis',
                                        'size':2,
                                        'colorbar':{'title':tally_name,
                                                    'tickvals':np.linspace(start=min(list(df_filtered_by_mat[tally_name])),stop=max(list(df_filtered_by_mat[tally_name])),num=10)
                                                        }
                                        }
        ))
